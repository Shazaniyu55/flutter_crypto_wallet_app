...
// ignore_for_file: file_names, library_prefixes, avoid_print, non_constant_identifier_names, unnecessary_string_interpolations, avoid_function_literals_in_foreach_calls, prefer_interpolation_to_compose_strings

import 'dart:ui';

import 'package:benex/models/coinsModel.dart';
...
import 'package:benex/services/dataService/dataService.dart';
import 'package:benex/services/sharedPrefs.dart';
...
class ApiService {
  ...
  Future withdrawCrypto({String? amount, String? coinId,String? networkId, String? address}) async {
    ...
    if (response!.statusCode! <= 201) {
      if (response.data != null) {
        UtilService().showToast('Withdraw Successful!',color: const Color(0xFF00D339));
      }
      res = 'OK';
  ...
  Future swap({String? amount, String? coinId,String? networkId,}) async {
    ...
    if (response!.statusCode! <= 201) {
      if (response.data != null) {
        UtilService().showToast('Swap Successful!',color: const Color(0xFF00D339));
      }
      res = 'OK';
  ...
  Future<Object> verifyOtpForForgotPassword({required String email, required String code}) async {
    ...
    if (response!.statusCode! <= 201) {
      ...
      UtilService().showToast('Otp Verified Successfully!',color: const Color(0xFF00D339));
    } else {
      appController.verifyOtpLoader.value = false;
    }
    return res;
  ...
  Future<Object> forgotPassword({required String email}) async {
    ...
    if (response!.statusCode! <= 201) {
      ...
      UtilService().showToast('Otp sent to $email!',color: const Color(0xFF00D339));
    } else {
      appController.forgotPassLoader.value = false;
    }
    return res;
  ...
  resetPass({required String pass}) async {
    ...
    if (response!.statusCode! <= 201) {
      ...
      UtilService().showToast('Password Updated Successfully!',color: const Color(0xFF00D339));
    } else {
      appController.resetPassLoader.value = false;
    }
    return res;
  ...
  Future deleteProfile() async{
    ...
    if(response!.statusCode! <= 201){
      if(response.data != null) {
        ...
        res = 'OK';
        UtilService().showToast('Profile successfully deleted!', color: const Color(0xFF00D339));
