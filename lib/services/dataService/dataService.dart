// ignore_for_file: file_names, prefer_final_fields, prefer_typing_uninitialized_variables, unused_local_variable, unnecessary_string_interpolations, prefer_interpolation_to_compose_strings, prefer_if_null_operators, deprecated_member_use
...
class DataService {
  ...
  Future<Response?> genericDioGetCall(String api) async {
    await getToken().then((String? token) async {
      var dio = Dio(BaseOptions(baseUrl: _baseURL, headers: {
        "Authorization": "$token",
        "Content-Type": "application/json"
      }));

      try {
        _response = await dio.get(_baseURL + '$api');
      } on DioError catch (e) {
        if (e.response != null) {
      ...
      if (e.response != null) {
        if (e.response!.data != null) {
          if (e.response!.data['message']
              .contains('User account is already linked with')) {
          } else {
  ...
  uploadIMAGES(String api, File frontProof, File backProof,
      String documentNumber, bool isAddressImage) async {
    var res = '';
...
Future<String?> getTokenForVerifyOtp() async {
  final SharedPreferences prefs = await SharedPreferences.getInstance();
  return prefs.getString('authTokenForReset') != null
      ? prefs.getString('authTokenForReset')
      : '';
}
