// ignore_for_file: must_be_immutable, sort_child_properties_last, unnecessary_string_interpolations, sized_box_for_whitespace, use_super_parameters
...
class _SwapState extends State<Swap> {
  ...
  void initState() {
    ...
    if (widget.token != null) {
      ...
      if (appController.userBalance.value.balance != null &&
          appController.userBalance.value.balance!.isNotEmpty) {
        fromCoin.value =
            appController.userBalance.value.balance?[0] ?? Balance();
      } else {
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    CommonWidgets().appBar(
                        hasBack: widget.showBack ?? false, title: 'Swap'),
                  ],
                ),
              ),
              Expanded(
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Padding(
                                            ...
                                            child: Row(
                                              children: [
                                                appController.getBalanceLoader
                                                            .value ==
                                                        true
                                                    ? Column(
                                                        children: [
                                            ...
                                            children: [
                                              Padding(
                                                padding: const EdgeInsets.only(
                                                    left: 9.0),
                                                child: Text(
                                                  appController.platFormFeeList
                                                          .isNotEmpty
                                                      ? "${appController.getFeeLoader.value == true ? "Loading..." : "${userGets.value}"}"
                                                      : "0.0",
                                                  style: TextStyle(
                                                      fontFamily: 'sfpro',
                                                      fontStyle:
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              appController.getFeeLoader.value == true
                                  ...
                                  : Text(
                                      (toCoin.value.id != null ||
                                              toCoin.value.id == '')
                                          ? ""
                                          : appController
                                                  .platFormFeeList.isEmpty
                                              ? ''
                                              : '${appController.platFormFeeList[appController.platFormFeeList.indexWhere((element) => element.feeName == 'swap_fee')].feePercentage ?? 0} %',
                                      style: TextStyle(
                                          fontFamily: 'sfpro',
                                          fontStyle: FontStyle.normal,
                                          fontWeight: FontWeight.w400,
                      ...
                      BottomRectangularBtn(
                          ...
                          onTapFunc: () {
                            if (amountController.text != '' &&
                                num.parse(amountController.text) > 0) {
                              ApiService()
                                  .swap(
                                      coinId: fromCoin.value.coinId,
                                ...
                                setState(() {});
                                ApiService()
                                    .getWalletWithBalance(showLoader: true);
                              });
                            } else {
                              UtilService().showToast('Amount can not be 0.',
                                  color: Colors.red);
                            }
                          },
                          btnTitle: 'Swap')
  ...
  Future _selectTokenBottomSheet(BuildContext context,
      {List<Balance>? wList, String? type}) {
                      ...
                      return toCoin.value.id == wList?[index].id
                          ...
                          : Padding(
                              ...
                              child: _tokenItems(
                                  item: wList?[index],
                                  amountInUSD:
                                      "${UtilService().toFixed2DecimalPlaces((wList?[index].balanceInLocalCurrency ?? 0).toString(), decimalPlaces: 4)}",
                                  coinName: "${wList?[index].symbol}",
                                  amountInEth:
                                      "${UtilService().toFixed2DecimalPlaces((wList?[index].balance ?? 0).toString(), decimalPlaces: 4)}",
                                  imgName: "${wList?[index].logoUrl}",
                                  percentage:
                                      "${UtilService().toFixed2DecimalPlaces((wList?[index].priceChange ?? 0).toString(), decimalPlaces: 2)}",
                                  priceInUSD:
                                      "${UtilService().toFixed2DecimalPlaces((wList?[index].price ?? 0).toString(), decimalPlaces: 4)}",
                                  type: type),
                            );
                    } else {
  ...
  getSwapQuote() async {
    if (amountController.text.trim() == '') {
      ...
      if (num.parse(amountController.text) > 0) {
        if (appController.getFeeLoader.value == false &&
            appController.platFormFeeList.isNotEmpty) {
          userGets.value = UtilService().toFixed2DecimalPlaces(
              ((num.parse(amountController.text) *
                          (fromCoin.value.price ?? 0)) -
                      ((num.parse(amountController.text) *
                              (fromCoin.value.price ?? 0)) *
                          num.parse(((appController
                                          .platFormFeeList[appController
                                              .platFormFeeList
                                              .indexWhere((element) =>
                                                  element.feeName ==
                                                  'swap_fee')]
