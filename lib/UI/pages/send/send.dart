// ignore_for_file: non_constant_identifier_names, deprecated_member_use, use_super_parameters

import 'package:benex/UI/pages/send/qrScanner.dart';
import 'package:benex/models/sendResponseModel.dart';
...
class Send extends StatefulWidget {
  const Send({Key? key, required this.selectedToken}) : super(key: key);
...
class _SendState extends State<Send> {
                    ...
                    Column(
                      children: [
                        BottomRectangularBtn(
                          onTapFunc: () {
                            if (appController.withdrawCryptoLoader.value ==
                                false) {
                              verifyFields();
                            }
                          },
  ...
  verifyFields() async {
    if (amountController.text.trim() == '') {
    ...
    } else if (double.parse(amountController.text) <= 0.0 ||
        double.parse(amountController.text) >
            double.parse((widget.selectedToken.balance ?? 0).toString())) {
      ...
      ApiService()
          .withdrawCrypto(
              ...
              coinId: widget.selectedToken.coinId,
              networkId: widget.selectedToken.networkId)
          .then((value) {
        if (value != null) {
